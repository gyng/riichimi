...
- Complete: deterministic local blur, glare, edge-crop, excessive-perspective, and frame-tilt diagnostics with specific retry guidance and manual escape paths. Measured against synthetically warped fixtures, keystone perspective degrades gracefully (all 15 tiles located to 60%, confidence falling), while in-plane rotation is the hard limit: the row band is found by scanline, so a turn past about 4° loses it entirely. That failure now names the angle instead of ending in a generic retry.
