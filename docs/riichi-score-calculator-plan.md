...
- Complete: deterministic local blur, glare, edge-crop, excessive-perspective, and frame-tilt diagnostics with specific retry guidance and manual escape paths, plus automatic levelling before the read. Measured against synthetically warped dogfood fixtures: keystone perspective degrades gracefully (all 15 tiles located to 60%, confidence falling), and in-plane rotation — which used to fail outright past about 4°, because the row band is found by scanline — now reads in full at 8° and 12°. A turn too far to rescue names its own angle rather than ending in a generic retry.
- Not yet measured: the scanner has never been run against a guided capture taken on a real phone. The seventeen personal photographs held for this purpose are whole-table game snapshots — walls, discards, sticks, the room — and are a source of tile crops for the classifier, not end-to-end scan inputs. A representative set of _guided_ captures across physical tile sets remains the outstanding evidence.
