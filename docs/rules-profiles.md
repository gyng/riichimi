...
The baseline profile follows the options represented by Riichimi's versioned [WRC 2025 reference](https://www.worldriichi.org/wrc-rules): open tanyao, kiriage mangan, kazoe capped at yakuman, double yakuman, renhou as mangan, and no red fives.

## WRC 2025 · red-five table

This is an explicit **Riichimi local profile**, not a separately published WRC ruleset. It preserves every baseline WRC 2025 option and changes exactly one policy: the three red fives are enabled and count as dora. The UI labels this difference beside the selector so a table cannot accidentally imply official WRC red-five rules.
