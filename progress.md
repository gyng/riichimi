...
- **Checkpoint selection uses the wrong distribution, and it barely matters.** Training keeps the epoch that scores best on synthetic vector art — 99.4%, saturated. The per-fold oracle gap is 2.7–3.0 points, but the aggregate curve shows real accuracy _plateauing_ from epoch 5 rather than peaking, so a realistic fix is worth about **two crops in 107** — inside the interval. Recorded as a correctness fix, not an accuracy lever; an earlier draft of this checkpoint quoted the oracle gap as if it were a gain, which it is not. `majiang2` never exceeds 0.333 at any epoch, so no training-schedule change touches generalization.
- **Trained a candidate on the corrected corpus and did not promote it.** Identical settings, scored on the same held-out 46: top-1 0.9348, top-3 1.0000, coverage 0.7826, accepted accuracy 1.0000, zero confident-wrong, 3.26 reviews per hand — **identical to the shipped model on every metric the gate reads**, though the three errors moved to a different three tiles. A defect was found in 31% of the physical training data, corrected, and retrained, and the release metric did not move by one crop. That is what the corpus problem costs, demonstrated rather than argued.
