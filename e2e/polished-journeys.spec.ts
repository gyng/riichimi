...
test("dogfoods visible desktop scoring and camera recovery without an agent", async ({ page }) => {
  ...
  await page.goto("/");
  // The tab icon ships as a vector, with a raster alternate for browsers that
  // will not take one. Both are served from the build, not generated by a
  // toolchain step.
  ...
  // The reference photo is an image, so its accessible name is its alt text.
  await expect(page.getByAltText("Captured hand reference")).toBeVisible();
